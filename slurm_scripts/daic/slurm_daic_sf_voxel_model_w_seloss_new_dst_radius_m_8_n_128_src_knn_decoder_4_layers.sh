...
#SBATCH --job-name="sf_voxel_model_with_seloss_new_dst_radius_m_16_n_128_src_knn"
...
#SBATCH --output=slurm_logs/slurm_sf_voxel_model_with_seloss_new_dst_radius_m_16_n_128_src_knn_%j.out
#SBATCH --error=slurm_logs/slurm_sf_voxel_model_with_seloss_new_dst_radius_m_16_n_128_src_knn_%j.err
...
srun python train.py model=sf_voxel_model lr=2e-4 epochs=20 batch_size=4 model.target.use_bn_in_vol=True model.target.m=16 model.target.n=128 model.target.decoder_layers=4 loss_fn=seflowLoss exp_note="with_seflowLoss_new_dst_radius_m_8_n_128_knn_src_4_layer_decoders" wandb_mode="online" gpus="auto" "add_seloss={chamfer_dis: 1.0, static_flow_loss: 1.0, dynamic_chamfer_dis: 1.0, cluster_based_pc0pc1: 1.0}" "model.val_monitor=val/Dynamic/Mean"
