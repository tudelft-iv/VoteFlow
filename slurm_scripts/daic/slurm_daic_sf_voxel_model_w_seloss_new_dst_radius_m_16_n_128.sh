...
srun python train.py model=sf_voxel_model lr=2e-4 epochs=20 batch_size=4 model.target.use_bn_in_vol=True model.target.m=16 model.target.m=128 loss_fn=seflowLoss exp_note="with_seflowLoss_new_dst_radius" wandb_mode="online" gpus="auto" "add_seloss={chamfer_dis: 1.0, static_flow_loss: 1.0, dynamic_chamfer_dis: 1.0, cluster_based_pc0pc1: 1.0}" "model.val_monitor=val/Dynamic/Mean"
