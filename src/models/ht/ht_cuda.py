...
class HT_CUDA(nn.Module):
    ...
    """ mapping from points to histgrams
        input: feats_src [b, l, c]
        input: feats_dst [b, l, c]
        ...
        input: bins_y int 
        input: bins_z int
        output: HT feaures [b, l, c*2, h, w], c*2 stands for the concatenation of features from src and dst.
    """
    ...
    def forward(self, feats_src, feats_dst, voxels_src, voxels_dst, idxs_src, idxs_dst):
        ...
        assert feats_src.shape == feats_dst.shape
        assert voxels_src.shape == voxels_dst.shape

        b, l, n = idxs_dst.shape
        _, _, m = idxs_src.shape

        datatype = feats_src.dtype
        assert feats_src.dtype==feats_dst.dtype
        ...
        vol = self.ht(feats_src, feats_dst, voxels_src, voxels_dst, idxs_src, idxs_dst, self.h, self.w, self.d)
